...
namespace AbsenceManager.DynamicData.CustomPages.TrackUsers
{
    public partial class List : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            long userID = 0;
            string userInfo = "";
            if(long.TryParse(Request.Params.Get(0), out userID))
            {
                using(AM_Entities ent = new AM_Entities())
                {
                    User u = (from user in ent.Users where user.ID == userID select user).FirstOrDefault();
                    userInfo = u.Nome + " (" + u.NrFuncionario + ")";
                }
                 
            }
            Title = table.DisplayName + " " + userInfo;
