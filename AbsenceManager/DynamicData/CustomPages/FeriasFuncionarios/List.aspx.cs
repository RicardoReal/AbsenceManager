...
namespace AbsenceManager.DynamicData.CustomPages.FeriasFuncionarios
{
    public partial class List : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            _hasTrackChangesPermission = gprp.UserCanTrackChanges(table.Name, userName);
        ...
        protected void GridView1_ItemCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                else if (e.CommandName == "Aprov")
                {
                    using (AM_Entities ent = new AM_Entities())
                    {
                        ...
                        FeriasFuncionario ff = ent.FeriasFuncionarios.Where(x => x.ID == id).FirstOrDefault();
                ...
                else if (e.CommandName == "NaoAprov")
                {
                    using (AM_Entities ent = new AM_Entities())
                    {
                        if (Int32.TryParse((string)e.CommandArgument, out index))
            ...
            {
                AppUtils.Utils.WriteErrorLog(ex.Message, ex.InnerException, ex.StackTrace, "FeriasFuncionariosList");
            }
        }
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                using (AM_Entities ent = new AM_Entities())
                {
                    long id = (long)GridView1.DataKeys[e.Row.RowIndex].Value;
                    bool pendente = ent.FeriasFuncionarios.Where(x => x.ID == id).FirstOrDefault().Pendente;

                    Button aprov = (Button)e.Row.FindControl("ButtonAprov");
                    Button nAprov = (Button)e.Row.FindControl("ButtonNaoAprov");

                    if (!pendente)
                    {
                        aprov.Enabled = false;
                        nAprov.Enabled = false;
                    }
